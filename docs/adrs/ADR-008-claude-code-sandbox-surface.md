...
## Amendment 2 — 2026-08-01 — C3 narrowed, A15 and A16 added, two open requirements added

Three observations made while verifying Amendment 1 change the record.

**C3 narrows and one of its two branches is excluded.** The probe repository's initial commit carries a good signature from an untrusted key, not an absent one. Signing was therefore in force and succeeded when that commit was written, which excludes the branch in which the signing configuration was bypassed. What remains is that the commit was written outside the strict profile, or under a profile without the credential deny. Its local configuration holds only the six default core entries and no user section, while its author and committer are a synthetic identity — so that identity came from the environment, not from local configuration, which is consistent with B6. The probe repository still cannot serve as evidence about repository initialisation under this profile, because nothing establishes which profile was in force when it was made. That question stands as an open requirement.

- **A15.** The repository test suite cannot complete under the strict profile. The guard fixture section passes in full; the suite then dies creating a temporary directory, because the temporary-directory utility resolves the platform's per-user temporary path rather than the value exported in the environment, and that path lies outside the profile's writable set. The failure text reports a permission error and names no policy layer, and the harness reports it as an ordinary test failure. The defect is structural rather than local: the same utility is invoked at twenty sites across the suite, so no single call site can be adjusted to avoid it.

This is the fourth instance of D4 in this document's own history, and the first with an adoption cost attached: a profile that cannot run the repository's own gates cannot be adopted for any unattended mode, regardless of what else it permits. The remedy is not a workaround at call sites. Either the profile's writable set covers the platform temporary path, or every gate is made to honour the exported temporary directory. That choice is deferred to the fixture specification and recorded here as an open requirement.

- **A16.** Signature verification is itself unavailable under the profile. Confirming that a commit is signed requires reading key material under the denied directory, so a session under this profile can produce neither commits nor verification of commits. The verification was performed by the operator outside the profile and is receipted in the evidence directory named in the ledger below: all three commits carrying this document verify as good signatures from a single key, and the transitions between commit-capable and commit-incapable sessions track the profile rather than any change in configuration.

**A receipt must name the profile it was taken under.** Evidence about a boundary is evidence about the boundary that was in force when it was captured. A denial captured under one profile and filed as a property of another is the measurement-basis defect of D1 committed at the level of the individual artifact. Every receipt in this document's evidence set therefore carries a header naming its profile, or the string recording that the profile could not be determined.

**Method note against L-033.** Amendment 1 asserted that both commits verify against the key at the configured path. That assertion was written before the verification existed and could not have been performed by the session that wrote it. The verification was subsequently produced outside the profile and is now receipted. The claim was true; the evidence for it was not yet in hand when it was made. Recorded because the tiering discipline in D6 is worth nothing if it is applied only to other people's claims.

Ledger. Narrows C3 by excluding its signing-bypass branch and retains its open requirement about which profile was in force. Adds A15 and A16. Adds two open requirements: reconciling the profile's writable set with the platform temporary path, and establishing the probe repository's provenance. Closes none. Evidence for A16 is stored under the repository's evidence directory, dated this day. This amendment is docs-only; the document's status remains Proposed.
